...
class Paramchange:
    def __init__(self):
        try:
            os.makedirs(entry_local_working_dir.get())
        except WindowsError:
            pass
        self.mapfile = entry_local_working_dir.get()+'MAP.xlsx'
        self.BSC = []
        self.BCF = []
        self.BTS = []
        self.TRX = []
    ...
    def writer(self,commands):
        cmd = commands
        fp = open(entry_local_working_dir.get()+"out.txt","w+")
        for c in cmd:
            fp.writelines(c+"\n")
        fp.close()
        os.startfile(entry_local_working_dir.get()+"out.txt")

    def readxl(self, filename, in_sheet_index):
        self.filename = filename
        wb = xlrd.open_workbook(self.filename)
        sh1 = wb.sheet_by_index(in_sheet_index)
    ...
    def GuiMap(self):
        ...
        for p in self.PARAMNAME:
            if p not in ALLPARAM:
                ...
                if a=="ok":
                    os.startfile(entry_local_working_dir.get()+"MAP.xlsx")
                    print "Good Bye!!!"
                    sys.exit()
        print "-----^-------------------------------\n"
        info = ""
