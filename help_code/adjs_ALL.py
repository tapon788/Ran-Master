...
class managedObject:
    def __init__(self):
        self.paramName = ["name","AdjsMCC","AdjsMNC","AdjsLAC","AdjsCI","AdjsRAC","AdjsRNCid","AdjsScrCode",
        "AdjsCPICHTxPwr","AdjsEcNoOffset","AdjsTxDiv","AdjsTxPwrRACH","AdjsDERR","AdjsSIB","RtHopsIdentifier",
        "NrtHopsIdentifier","HSDPAHopsIdentifier","RTWithHSDPAHopsIdentifier"]



    def mo_handler_createNew(self,name,AdjsLAC,AdjsCI,AdjsRAC,AdjsRNCid,AdjsScrCode,AdjsCPICHTxPwr,AdjsEcNoOffset,AdjsTxDiv,AdjsTxPwrRACH,AdjsDERR,AdjsSIB,RtHopsIdentifier,NrtHopsIdentifier,HSDPAHopsIdentifier,RTWithHSDPAHopsIdentifier):

        array_length = len(name)
        AdjsMCC = ["470"]*array_length
        AdjsMNC = ["03"]*array_length
        ...
        data_array = (adjsname,AdjsMCC,AdjsMNC,AdjsLAC,AdjsCI,AdjsRAC,AdjsRNCid,AdjsScrCode,AdjsCPICHTxPwr,AdjsEcNoOffset,AdjsTxDiv,
                        AdjsTxPwrRACH,AdjsDERR,AdjsSIB,RtHopsIdentifier,NrtHopsIdentifier,HSDPAHopsIdentifier,RTWithHSDPAHopsIdentifier)
        ...
        fp = open("D:\\3gext\NewADJS_create.xml","w+")
    ...
    def readinput_NAC(self):
        raw_NAC_input = []
        fp = open("D:\\3gext\inputADJS.csv")

        for line in fp.readlines():
            raw_NAC_input.append(line.strip())
        fp.close()
    ...
    def paramArray_NAC(self,in_raw_NAC_input):
        ...
        for i in in_raw_NAC_input[1:]:
            ...
            adjsname.append(i.split(",")[3])
            AdjsLAC.append(i.split(",")[6])
            AdjsCI.append(i.split(",")[7])
            AdjsRAC.append(i.split(",")[8])
            AdjsRNCid.append(i.split(",")[9])
            AdjsScrCode.append(i.split(",")[10])
            AdjsCPICHTxPwr.append(i.split(",")[11])
            AdjsEcNoOffset.append(i.split(",")[12])
            AdjsTxDiv.append(i.split(",")[13])
            AdjsTxPwrRACH.append(i.split(",")[14])
            AdjsDERR.append(i.split(",")[15])
            AdjsSIB.append(i.split(",")[16])
            RtHopsIdentifier.append(i.split(",")[17])
            NrtHopsIdentifier.append(i.split(",")[18])
            HSDPAHopsIdentifier.append(i.split(",")[19])
            RTWithHSDPAHopsIdentifier.append(i.split(",")[20])

        return(name,AdjsLAC,AdjsCI,AdjsRAC,AdjsRNCid,AdjsScrCode,AdjsCPICHTxPwr,AdjsEcNoOffset,AdjsTxDiv,AdjsTxPwrRACH,AdjsDERR,
        AdjsSIB,RtHopsIdentifier,NrtHopsIdentifier,HSDPAHopsIdentifier,RTWithHSDPAHopsIdentifier)
        pass
...
class sql_handler:
    ...
    def setAdjgID(self,in_name):
        ...
        if plmn not in wcel:
            ...
            plmnID.append(plmn+"/ADJS-"+str((d[0])))
...
if __name__ == '__main__':
    ...
    if operation_type=="EXT":
        ...
        prm_ary_NAC[7],prm_ary_NAC[8],prm_ary_NAC[9],prm_ary_NAC[10],prm_ary_NAC[11],prm_ary_NAC[12],prm_ary_NAC[13],
        prm_ary_NAC[14],prm_ary_NAC[15]
        )
